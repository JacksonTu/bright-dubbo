package com.tml.common.redis.utils;
...
public class RedisUtil {
    ...
    public void setEx(String key, Object value, Long seconds, TimeUnit timeUnit) {
        valueOps.set(key, value, seconds, timeUnit);
    }
    ...
    public Set<String> scan(String keyPrefix) {
        ...
        try {
            Cursor<Map.Entry<Object, Object>> cursor = redisTemplate.opsForHash().scan(keyPrefix,
                    ScanOptions.scanOptions().match("*").count(1000).build());
            while (cursor.hasNext()) {
                Map.Entry<Object, Object> entry = cursor.next();
                Object key = entry.getKey();
                Object valueSet = entry.getValue();
                keys.add(valueSet.toString());
            }
            //关闭cursor
            cursor.close();
        } catch (Exception e) {
